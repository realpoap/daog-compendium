...
const CharacterVariablesPanel = ({ char }: Props) => {
				...
				char.specifics.alignment === 'bad'
					? char.variables.intimidation > char.variables.persuasion
						...
						: `CHA ${
								...
								char.profile.boni.persuasion + char.profile.variables.persuasion
	...
	return (
		<div
			className={cn(
				`xs:grid-cols-7 group m-2 grid grid-cols-5 gap-2 font-bold`,
				{
					'grayscale-50 opacity-50': char.bio.isDead,
				},
			)}
		>
			{masteriesObjects.map(
				({ icon: Icon, label, value, name, stat }, index) => (
					<div
						className='tooltip'
						key={index}
					>
