...
export const ZodNewSpell = z.object({
  ...
  errorMap: () => ({ message: 'A spell shall have a type' })}),
	...
	action: z.enum(['charm', 'damage', 'heal', 'protect', 'enhance', 'link', 'create', 'transform', 'move', 'remove', 'puzzle'],{
  errorMap: () => ({ message: 'A spell shall do something' })}),
	targetType: z.enum(['single', 'multiple', 'random', 'self', 'none'],{
  errorMap: () => ({ message: 'A spell shall have a target' })}).nullable(),
	casting: z.enum(['instant', 'delayed', 'ritual', 'concentration', 'upkeep'],{
  errorMap: () => ({ message: 'A spell must be casted' })}).nullable(),
	description: z.string({message:'Wizards are no seer : tell us the secrets of this magic !'}),
